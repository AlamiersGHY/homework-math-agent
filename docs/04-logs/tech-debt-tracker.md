...
| --- | --- | --- | --- | --- | --- | --- |
| TD-001 | Evals | Eval runner is not implemented yet; eval cases are currently static JSON. | Agent behavior cannot be automatically scored yet. | Add runner after backend or scripts scaffold exists. | Phase 1 | open |
| TD-002 | Scripts | Project-level `dev`, `test`, `eval`, `check`, and `release-check` commands do not exist yet. | Agents must still use app-local commands until wrappers are added. | Add scripts now that `apps/web` and `apps/api` are scaffolded. | Phase 1 | open |
